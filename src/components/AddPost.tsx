...
const AddPost: FC<AddPost> = ({ onCloss, post }) => {
  ...
  const [data, setdata] = useState<PostForm>({
    title: post?.title || "",
    body: post?.body || "",
    image: null,
  });

  const [imgePost, setimgePost] = useState<string>(post?.image || "");

  const handelImgePost = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    ...
    >
      <Box onClick={onCloss} sx={{ display: "flex", alignSelf: "flex-end" }}>
        <CloseIcon
          onClick={onCloss}
      ...
      {!imgePost && (
        <label htmlFor="upload-image">
          <Button
            ...
            component="span"
            color="primary"
            sx={{ borderRadius: "50%", height: "100px", mb: 3, width: "100px" }}
          >
      ...
      <TextField
        onChange={(event) => {
          setdata((prve) => ({ ...prve, body: event.target.value }));
        }}
        autoFocus
